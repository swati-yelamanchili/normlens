...
benchmarking = BenchmarkingEngine(embedding_service, classifier)
outlier_detector = OutlierDetector(embedding_service, classifier, benchmarking)
...
def analyze_contract(
    ...
    try:
        ...
        raw_clauses = segmenter.segment(parsed["text"], parsed.get("pages"))
        ...
        for clause_model in clause_models:
            clause_type, confidence = classifier.classify_best(clause_model.clause_text)
        ...
        for clause_data in classified_clauses:
            attr = extractor.extract(clause_data["clause_text"], clause_data["clause_type"])
            ...
            findings = risk_engine.evaluate_clause(clause_data, attr)
            all_findings.extend(findings)

            if clause_type:
                outliers = outlier_detector.detect_outliers(clause_data, attr, clause_type)
                all_outliers.extend(outliers)

                benchmarks = benchmarking.benchmark_attributes(
            ...
            clause_model.attributes = attr
            db.commit()

        missing_findings = risk_engine.evaluate_missing_clauses(
        ...
        total_score = risk_engine.calculate_risk_score(all_findings)
        risk_level = risk_engine.get_risk_level(total_score)
